...
| C-09 | classifyMediaError 6분류 | `lifecycle:error{reason: mic_<category>}` denied/not_found/busy/constraint/unknown | `50_lifecycle §classifyMediaError` | ⬜ |
| C-10 | classifyPcError 3분류 | `lifecycle:error{reason: pc_<category>}` ice/dtls/unknown | `50_lifecycle §classifyPcError` | ⬜ |
...
| C-14 | ACK timeout 10s / pending 64 초과 → reconnect | `signaling._scheduleReconnect`, `reconnect:attempt` | `21_opcode §흐름제어` | ⬜ |
