...
| G3-02 | `setMicCapture({AGC, NS, EC})` 1차 applyConstraints | track.getSettings 일치, return 'applied' | 동일 | ⚠️ |
| G3-03 | `setMicCapture` fallback (Firefox 등) → 재획득 + swapTrack | return 'reacquired' | 동일 | ⚠️ |
...
> ⚠️ G3-02 / G3-03 ret='reacquired' 이지만 **실제 capture constraint 미반영** — `{AGC:false, NS:true, EC:true}` 호출 후 track.getSettings 가 여전히 `{AGC:true, NS:true, EC:true}`. 새 track 으로 swap 됐는데 constraint 가 적용 안 됨. Chrome 정책 또는 SDK 결함 의심. README §F 등록.
