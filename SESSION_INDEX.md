...
> 최종 업데이트: 2026-06-03 (Phase 132 Cross-SFU Phase 2a — hub room 라우팅: room_sfu(DashMap RoomId→SfuId)+place_room(RoundRobin)+sfu_for_room(매핑없음 None=폴백X)+all_sfu_clients(ROOM_LIST fan-out). ★assign_room=entry().or_insert_with(place_room) 로 동시 same-id CREATE TOCTOU race(방 split) 봉합. ws ROOM_CREATE 배치/generic dispatch sfu()→sfu_for_room. sfu() 유지(events 2b). 합격 ① 분산 ② 라우팅 ③ 회귀 실측 PASS. test oxhubd 24·common 24·oxsfud 206. `745edf8` / Phase 131 Cross-SFU Phase 2 선행 — sfud ROOM_CREATE 멱등(ensure: 명시 id 존재 시 기존 방 ok, 2006 거부 폐기) + default rooms(demo_*/qa_*) 제거 → room_id 전역 유일(1방 1sfu). oxrtc connect_and_join 에 CREATE+JOIN. 합격 ① 멱등 단위시험 ② 회귀 conf_basic/ptt_rapid PASS. test oxsfud 206. 백로그(부장): signal_client 분리·ptt_rapid floor flake. Phase 2a(hub 라우팅) 토대. `ca6fcf1` / Phase 130 Cross-SFU Phase 1 — hub sfu 레지스트리(단일 sfu 가정 해체). config `[[hub.sfu]]` 신규(+`[sfu]` 폴백 "sfu-1"), state `sfu_slot`→`sfu_registry` DashMap<id,SfuNode>+per-id lazy reconnect, **`sfu()` 시그니처 유지로 호출처 7곳 무변경**(하위호환=Phase 1 안전장치). 합격 ① 단일 config 회귀(admin/rooms 200) ② 2-entry 둘 다 connected. 라우팅(sfu_for_room)=Phase 2. test oxhubd 24·common 23·oxsfud 204. `da29ad8` / Phase 129 Cross-SFU Phase 0 — sfud CLI arg override(`--ws-port`/`--udp-port`/`--grpc-listen`/`--public-ip`, 수동 파싱·arg > toml > detect, grpc_listen 지역화) → supervisor 2-unit(sfud1 50051·sfud2 50052) 둘 다 live·다른 pid·충돌 없이 bind 입증(관측 i status 검증도구). 발견_사항: create_default_rooms 양쪽 동일 방 ID→Phase 2. test oxsfud 204 무영향. `c1a938f` / Phase 128 Supervisor 관측 레이어 — REST `/admin/supervisor/{status,restart}`(admin 인증, status=UnitStatus 배열·restart=202 위임) + healthz `live/ready`(무인증, enabled 전부 Live→200/503) + HubState attach(`#[cfg(unix)]` ArcSwap, sfu_slot 패턴·import 순환 없음). HTTP e2e 입증(status 401/200·restart 202+재기동). fix: admin kick 라우트 axum 0.7 path param `{}`→`:`(잠재 404). test oxhubd 24·common 20·oxsfud 204. `4bd5caa` / Phase 127 oxhubd Supervisor(Process Control Plane) Level 1 신설 — 자식 프로세스 lifecycle(spawn/ReadyCheck 4종/restart+Erlang intensity/kubelet backoff/stop Signal·Command/SIGKILL escalation) + hub 종료골격 신설(kill -TERM→supervisor.shutdown→sfud 연쇄 graceful 입증, 좀비 0). 선결 sfud SIGTERM(0603e `42476f7`) + 코어/intensity정정(record_start 폐기→crash-before-ready도 Blocked=빌드깨짐 차단)/Slave→Unit rename(f·g·h `c761543`). 불변원칙 3, 범위밖 REST/healthz/HubState attach=다음지침. test oxhubd 21·common 20·oxsfud 204·0 warning / Phase 126 naming_cleanup 꼬리 청소 — behavior 0(#5 streams 변수→tracks[도메인 어휘 61건 보존, ingress_rtcp 0건] + #4 dead variant NotPttRoom 제거[ping은 LIVE 전제 정정], test 204 불변·0 warning, c36cfc8) / Phase 125 SubscriberStream.virtual_ssrc→vssrc — 식별 계층 vssrc 통일 마무리, behavior 0(단일 필드+읽는자리 12건 E0609 전수, rewriter/Slot 어휘직교 비대상 무손상, test 204 불변, 6775b06) / Phase 124 식별자/주석 청산 — behavior 0(affiliate/deaffiliate·peer_map·find_publisher_by_ssrc·sync_scope_on_join/_leave rename + stale 주석 청산, 26파일, test 204·oxe2e 4/4, fa365f8) / Phase 123 PROJECT_MASTER 3파일 분리(MASTER/SERVER/WEB) + 0602e 현행화 — 코드 종속 격리, 손실 0, 071cef5 / Phase 122 Track State 통일+식별 계층 재설계 서버 A·B — track_id(불투명)·vssrc(값) 논리 PublisherStream 이주, mute/duplex Stream 단위, 응답 d.tracks(self-unicast 폐기). test 204, oxe2e 4/4. 정정 bf51697(active 통지 track_id) + 회귀 강화 4efaf4a(judge track_id 검증+단위테스트). commit 8b0627a·208a498. 클라 별도 세션 / Phase 121 domain 파편·래퍼 정리(3558faf·8291ff0) / Phase 120 통계 트랙 정렬)
...
- **최종 업데이트**: 2026-06-03 (Phase 132: Cross-SFU Phase 2a — hub room 라우팅(설계 §7). state room_sfu/place_room(RoundRobin)/sfu_for_room(매핑없음 None=폴백X)/all_sfu_clients + ★assign_room(entry().or_insert_with(place_room) 원자화로 동시 same-id CREATE TOCTOU race=방 split 봉합, conf_basic 2봇 재현·입증). config [routing] placement. ws ROOM_CREATE 배치+기록·ROOM_LIST fan-out·dispatch/forward/cleanup sfu()→sfu_for_room. sfu() 유지(events 2b). 합격 ① 분산(qa_test_01→sfu-1·02→sfu-2) ② 라우팅(각 sfu PASS·매핑없음→SfuUnavailable) ③ 회귀(단일 폴백 PASS). test oxhubd 24·common 24·oxsfud 206. 백로그(부장): sfu 영구다운 죽은매핑 재배치. Phase 2b=event 복수화+sfu() 폐기. `745edf8` / Phase 131: Cross-SFU Phase 2 선행 — sfud ROOM_CREATE 멱등(ensure get-or-create) + default rooms 제거(room_id 전역 유일=1방 1sfu, 설계 §7.2/7.3). room_ops 명시 id 존재→기존 방 ok(2006 폐기)·name 검증 생성 경로 이동, startup create_default_rooms 제거, oxrtc connect_and_join CREATE+JOIN(봇, 호출처 oxe2e 1곳). 합격 ① 멱등 단위시험(AppState ServerCert+UdpSocket 신규) ② 회귀 conf_basic PASS·ptt_rapid PASS. test oxsfud 206. 발견_사항: error.rs 2006 dead·demo 웹 Phase 3. 백로그(부장 기록): signal_client connect/create/join 분리·ptt_rapid floor flake 1/4. Phase 2a(hub room_sfu/place_room/sfu_for_room/라우팅) 토대. `ca6fcf1` / Phase 130: Cross-SFU Phase 1 — hub sfu 레지스트리(단일 sfu 가정 해체, 설계 §6). config 두 계층: `[sfu]`(sfud self, 무변경) + `[[hub.sfu]]`(hub registry 신규)+`sfu_registry()` 폴백("sfu-1"). state `sfu_slot`(단일)→`sfu_registry` DashMap<id,Arc<SfuNode>>+`default_sfu_id`, `sfu_by_id(id)` per-id lazy reconnect, **`sfu()`=`sfu_by_id(default)` 시그니처 유지→호출처 7곳 무변경**(Phase 1 회귀 최소화 안전장치). main 복수 try_connect. 합격 ① 하위호환(`[[hub.sfu]]` 없는 config 단일 회귀·admin/rooms 200) ② 복수 dial(2 entry→sfu-1/sfu-2 connected). 시그니처 선조치: HubState::new(nodes) 호출처 main 1곳·SfuConfig(self) 무변경. test oxhubd 24·common 23(sfu_registry 3 신규)·oxsfud 204 무영향. 범위밖: room_sfu/sfu_for_room/배치/라우팅 전환=Phase 2. `da29ad8` / Phase 129: Cross-SFU Phase 0 — sfud CLI arg override(`arg_value` 수동 파싱, clap 미도입; `--ws-port`/`--udp-port`/`--grpc-listen`/`--public-ip`; 우선순위 arg > system.toml > detect; grpc_listen 지역화로 gRPC spawn 전달; override info 로그 tracing init 이후). 합격 판정: supervisor 2-unit(sfud1 default 50051 · sfud2 args override 50052) 둘 다 spawn→충돌 없이 bind→`/admin/supervisor/status` 2 live·다른 pid(관측 레이어 i 검증도구)·healthz/ready 200. 발견_사항: create_default_rooms 양쪽 동일 방 ID 생성→Phase 2(routing) 정리. oxsfud/lib.rs 단일 파일. test oxsfud 204·oxhubd 24·common 20 무영향. 범위밖: hub registry/`[[sfu]]`/routing=Phase 1~3. `c1a938f` / Phase 128: Supervisor 관측 레이어 — supervisor 코어 외부 조회/제어(설계 §12). A `status()→Vec<UnitStatus>`+`all_units_ready()`. B HubState attach(`#[cfg(unix)]` ArcSwap supervisor 필드, main 사후 store, shutdown=state load, import 순환 없음). C REST `/admin/supervisor/{status,restart/{alias}}`(admin 인증, 202 위임, non-unix stub). D healthz `live/ready`(무인증·크로스, enabled 전부 Live→200/503). HTTP e2e 입증(healthz 200·status 401/200·restart 202+새 pid 재기동). fix: admin kick 라우트 path param `{..}`→`:..`(axum 0.7 문법, 잠재 404). 신규 2+수정 7파일. test oxhubd 24·common 20·oxsfud 204·0 warning. `4bd5caa` / Phase 127: oxhubd Supervisor(Process Control Plane) Level 1 신설 — 자식 프로세스 control plane. 불변원칙 3(자식종류 무지·stop 양쪽·확장 enum). `supervisor/` 8파일: spawn(Stdio::inherit/kill_on_drop/process_group)→ReadyCheck 4종→Live→crash→restart(정책+Erlang intensity)→kubelet backoff→stop(Signal/Command+SIGTERM fallback)→SIGKILL escalation. hub main 종료골격 신설+배선 — hub `kill -TERM`→sfud 연쇄 graceful exit·좀비 0 입증(설계 §5-D). config `[supervisor]`/`[[units]]` DTO→UnitSpec, metrics 12필드. [g] intensity 카운트=재시작 시도시점(record_start 폐기→crash-before-ready Blocked) + 안정 Live backoff 리셋(STABLE_RESET_SEC=600) + 전역 config 제거. [h] Slave→Unit rename(systemd 정합). nix=cfg(unix) Windows 보존. 선결 0603e sfud SIGTERM `42476f7`. test oxhubd 21·common 20·oxsfud 204·0 warning. `c761543` / Phase 126: naming_cleanup 꼬리 청소 — behavior 0(#5 streams 변수→tracks[egress/ingress_publish, 도메인 어휘 61건 보존, ingress_rtcp 0건] + #4 dead variant NotPttRoom 제거[Floor::ping LIVE 전제 정정], test 204 불변·build 0 warning, `c36cfc8`) / Phase 125: SubscriberStream.virtual_ssrc→vssrc 식별 계층 vssrc 통일 마무리 — behavior 0(단일 필드 rename + 읽는자리 12건 컴파일러 E0609 전수, rewriter/Slot 어휘직교 비대상 무손상[타입 분리 함정], test 204 불변, `6775b06`) / Phase 124: 식별자/주석 청산 네이밍 정합 — **behavior 0**(로직·자료구조 무변경). affiliate/deaffiliate·sync_scope_on_join/_leave·find_publisher_by_ssrc·peer_map rename + stale 주석(TRACKS_ACK→READY·EndpointMap→PeerMap·floor Prepared 제거·SWITCH_DUPLEX→TRACK_STATE_REQ) 청산. 26파일 +144/−155. test 204 불변 + oxe2e 4/4. 별토픽 이관: A1 virtual_ssrc→egress_ssrc·SCOPE op 전면청산. `fa365f8` / Phase 123 PROJECT_MASTER 3파일 분리 + 0602e 현행화(9a2ec84·071cef5) / Phase 122 track_state 서버 A·B(8b0627a·208a498))
