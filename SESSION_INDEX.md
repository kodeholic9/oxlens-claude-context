...
| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0518e | `20260518e_oxlens_home_v3_migration_phase1_done` | 클라 | **클라 Signaling v3 진입** — wire.js 신규(encode/decodeFrame+ACK_STATE/PRIO)+constants OP v3 카테고리 nibble 교체+signaling.js binary frame 재작성. Pan-Floor(svc=0x03) 전체 폐기. 5 commits net -426. sdp-builder 82/82·wire 5/5 PASS. push 보류. |
...
| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0603 | `20260603o_reference_review_local_done` | 검토 | **새 클라 골격 근거 — mediasoup/LiveKit/Jitsi 코드레벨 정독**. Q1~Q5 표(연결추상화/경계/계층/재협상/트랙평등). LiveKit RTCEngine→PCTransportManager 가 우리 가설 동형. ★Q5: 3종 모두 송신평등+수신자유, **exclusive-send(PTT floor) 부재**=우리 차별점(송신 권한게이트+수신 floor분기+floor 1급화). 코드 0. |

## Phase 135: 새 SDK 재작성 Phase 1 — 골격(scaffold) (0603p)

| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0603 | `20260603p_client_rewrite_scaffold_done` | 클라 | **oxlens-home 웹클라 전면 재작성 골격**(설계 `20260603_client_rewrite_core_design`). `sdk/` 신설 29 .js, `core/` 보존. 11 폴더+engine.js facade. 본체=event-bus·env-adapter, 나머지 stub. `node import` load OK(exports 32). 의존 단방향·순환 0. core/서버 무영향. `39e22a1` |

## Phase 136: 새 SDK Phase 2 — Transport 본체 (publish 2a + subscribe·관측 2b) (0603q)

| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0603 | `20260603q_transport_publish_done` | 클라 | **Transport publish 본체**(구 sdp-negotiator 흡수). `transport.js` engine 의존 제거(bus.emit pc:* / opts). ensurePublishPc/addPublishTrack(bindSender)/enrichPublishIntent(sig.send 제거). MBCP 까기는 PTT 몫(미이식). `_t2a_check` PASS. core/ 무수정. |
| 0603 | `20260603q_transport_subscribe_done` | 클라 | **Transport subscribe·관측 본체**(완성). queueSubscribeRenego(★Promise 직렬화=glare 차단)/ontrack→`track:received`(물리 사실만)/collectStats(raw). ★정정: BWE=Telemetry 몫(Transport 는 collectStats 만). `_t2b_check` PASS+2a 회귀. core/ 무수정. |

## Phase 137: 새 SDK Phase 3a — domain 이식 + 수신(subscribe) 배선 (0603r)

| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0603 | `20260603r_domain_subscribe_wiring_done` | 클라 | **domain 3계층(Pipe/Endpoint/Room) 본체 + 수신 3점 배선**. bus+transport 주입. ① track:received=Room 직접 구독→matchPipeByMid→media:track ② queueSubscribeRenego ③ assignMids 이식. Pipe=core verbatim(필터 no-op). `_t3a_check` PASS. **발견**: 멀티룸 합집합 미구현→TransportSet 사유. core/ 무수정. |

## Phase 138: 새 SDK Phase 3b — domain publish + 타입 분리 + v2 잔재 청산 (0603s + 0604a 결함패치)

| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0603 | `20260603s_domain_publish_done` | 클라 | **전면 재작성 5계약 한방**(설계 §13). ①타입 분리(base Pipe+LocalPipe/RemotePipe+Local/RemoteEndpoint) ②signaling v2 청산(op switch 전멸→request pid Promise) ③track_id 1급(서버 응답 단일) ④송출 직렬화(replaceTrack(null) RTP 차단→ok→setTrack) ⑤MediaAcquire. ★Chrome 148 실증: replaceTrack(null) 후 ssrc 잔존. `_t3b1`/`_t3b2` PASS. core/ 무수정. |
| 0604 | `20260604a_publish_defects_done` | 클라 | **3b 결함 2건 패치**(`_publishOne`). 결함1(다중 video mid): 방어 가드(`if mid==null throw`). 결함2(에러 누수): reject 시 임시키 제거+deactivate+`publish:fail`+rethrow. `_t3b2` 확장 PASS. |

## Phase 139: 새 SDK Phase 3d — join orchestration (connect→join→hydrate→publish 글루) (0604b)

| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0604 | `20260604b_join_orchestration_done` | 클라 | **engine.js join 전체 흐름 배선**. ★v2→v3: ROOM_JOIN send+콜백 폐기→`await request` 단일. connect→handshake→identified / joinRoom→assembleRoom→hydrate(방안 A, 서버 track_id 단일)→join:ok. recv pipe 중복 0(matchPipeByMid). 범위=단일방 full-duplex. `_t3d_check` PASS. core/ 무수정. |

## Phase 140: 새 SDK Phase G — 라이브 2-peer E2E (★ 신규 sdk 첫 실 RTP 검증, 5/5 PASS) (0604c)

| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0604 | `20260604c_phase_g_live_e2e_done` | 클라 | **신규 sdk 첫 라이브 검증 — connect→ROOM_CREATE→JOIN→publish→subscribe 실 2-sfu 5/5 PASS**. harness `sdk/_e2e/`(수동), admin WS 교차검증. ★발견: ROOM_CREATE 선행 필수(qa_* 폐기) + ROOM_JOIN 응답=`{participants,tracks}`(stale 카탈로그 정정)→hydrate 키 패치. core/서버 무수정. |

## Phase 141: 새 SDK Phase 3e — 수신 경로 완결 (bob-first/leave/N-party, 라이브 4/4 PASS) (0604d)

| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0604 | `20260604d_recv_complete_done` | 클라 | **수신 경로 양방향+동적+다자 완결**(alice-first→3경로). 핵심=구독 배선(`tracks:update` emit 구독자 없던 구멍): room.js bus.on tracks:update/room:event. leave 2단계. `_t3e_check` PASS + **라이브 4/4**(alicefirst/bobfirst/leave/trio). 무수정. |

## Phase 142: 새 SDK Phase 3c — mute/unmute + setTrackState 단일 게이트(+duplex 흡수) (0604e + 0604f)

| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0604 | `20260604e_mute_done` | 클라 | **mute/unmute(full-duplex) — 첫 미디어 제어**. 송신=track.enabled 토글(replaceTrack 금지=SSRC 보존)+MUTE_UPDATE. 수신=Room track:state→RemotePipe avatar. half mute skip. ★라이브 SSRC 보존 입증. (구조 위반 2건은 0604f 교정) |
| 0604 | `20260604f_track_state_gate_done` | 클라 | **3c-fix: setTrackState 단일 게이트 + duplex 흡수**. 위반① setMuted→`setTrackState({muted?,duplex?})`(muted→MUTE_UPDATE/duplex→TRACK_STATE_REQ). 위반② 수신 active 누락→`_onTrackState` muted+active 둘 다+RemotePipe.setActive(#14 차단). `_t3c_check` PASS. 무수정. |

## Phase 143: 새 SDK PTT 서브시스템 Phase A~C — frame codec 분리 + floor + power (★정지점) (0605a)

| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0605 | `20260605a_ptt_subsystem_done` | 클라 | **PTT 서브시스템 골격(A~C)** — 코어 if(ptt)=0, ③훅. A=`dc-frame.js`+`mbcp.js`(죽은코드 청산)+transport TEMP import 해소. B floor.js(FloorFsm, transport.sendUnreliable 단일·bus.emit). C power.js(★정지점, bus.on floor:*/env:*·pipe suspend/resume, HOT/STANDBY/COLD). mock PASS. core/ 의존 0. |

## Phase 144: 새 SDK PTT Phase D~E — 조립 + half publish + harness ptt 시나리오 (0605a §7~11)

| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0605 | `20260605a_ptt_subsystem_done` (§7~11) | 클라 | **PTT 조립(D)+harness 시나리오(E)**. D=virtual.js(slot pipe)+freeze.js(taken→show/idle→hide, display:none 금지)+ptt.js 조립(순서 §8)+engine 배선. E=half-duplex publish 개통(half→simulcast off)+`?scenario=ptt`. `_ptt_check` PASS, 코어 if(ptt)=0. ★라이브 #3 PASS. 무수정. |

## Phase 145: 새 SDK observability + TransportSet — collectStats 경유 + status 평면 취합 + 멀티룸 합집합 (0605b)

| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0605 | `20260605b_observability_transportset_done` | 클라 | **관측 평면 2개 본체 + cross-sfu 물리축(TransportSet)**. Phase 0=`offChannelMessage`(floor.detach MBCP 누수 차단, `55dfd3a`). A=telemetry.js(collectStats 경유, 단방향). B=lifecycle.js(평면 status getter 취합). C(★정지점)=transport-set.js(`Map<sfuId,Transport>`)+engine `_renegotiateSfu`(같은 sfu 전 Room recv 합집합 1회). 단일 sfu 동형 보존. mock PASS. `fdedb76` |
...
| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0606 | `20260606a_uniform_ack_done` | 클라+서버 | **동작 0, 선언/주석 정리** — v3 대칭 ACK 의 거짓 no-ack 예외 청산. 실증: windowed 는 무조건 ack, 우회=채널 선택. A `wire.js` requiresAck 0x2500/0x1304 두 줄 삭제. B opcode.rs 주석. C wire_v3_catalog 정정. 규칙=\"윈도우 타면 ack, 우회하면 면제\". 회귀 PASS. home `dea7310`/server `0eed964` |
...
| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0606 | `20260606b_master_doc_refresh_done` | 문서 | **PROJECT_* 마스터 3종 현행화(doc-only)** — WEB 신 SDK 6평면+3훅 교체·MASTER 43op+CLIENT_EVENT·SERVER cross-sfu+supervisor 반영. 직접 정독 실증. ★부장 조치=claude.ai 지식 mirror 재업로드. |
| 0606 | `20260606b_client_api_categories` | 문서/분석 | **클라 SDK 외부 API 7 카테고리(C1~C7) 분류**(김대리). 업계 표면→이상적 노출 평면, `design/` 6종. C4 swapTrack↔half 결함·C6 EventBus facade화·C7 engine.scope(scope.js stub). 코드 0. |
| 0606 | `20260606c_repo_doc_resync_done` | 문서 | **oxlens-sfu-server repo 문서 현행화** — CLAUDE.md/README v2→v3(opcode 16진·domain/·2계층 Publisher·MBCP TLV·cross-sfu). README 포지셔닝 정정. `.env` 死파일 삭제. system.toml sfud2 udp→19741. |
| 0606 | `20260606d_category_crossverify` (→ `design/20260606_client_api_categories.md`) | 문서/분석 | **C1~C7 4열 대조표**(김과장) — mediasoup/livekit/jitsi/우리 소스 직접 추출, 빈칸 가시화. C7 PTT 독립 신설+Scope 편입. 경계판정 3건 닫힘. 코드 0. |
...
| 날짜 | 파일 | 영역 | 요약 |
|------|------|------|------|
| 0609 | `20260609a_outer_plane_lowrisk_cleanup_done` | 홈 SDK | **① 청소 G1·G6·G7** — MUTE_UPDATE→TRACK_STATE_REQ 단일 / `TrackState`→`LocalPipeState`+media SSOT(`sender.track`) / trackKey(복수 cam/screen). 동작 불변. `c3d5552` |
| 0609 | `20260609b_outer_plane_g2_g5_done` | 홈 SDK | **② 다방 본체 G2~G5** — duplex A안(power attach/detach)·`_upstreamPaused` 폐기 / 발언축1·청취축N(viaRoom→`room.on('speaker')`) / scope affiliate·select·join(pub⊆sub) / engine room_id 라우터. 서버게이트 S1✅·S2/S3❌. `a52d02d` |
| 0609 | `20260609c_server_g1_s2_done` | 서버 | **서버 정합** — TRACK_STATE_REQ muted 통합(`do_mute` 공유) + SCOPE `pub_select`(`publish.select`, 응답 pub_rooms 확정). cargo+oxe2e 4종 PASS. `60e6dba` |
| 0609 | `20260609d_outer_plane_followup_done` | 홈 SDK | **후속 A·B** — A2 거짓주석 정정 / B cross-sfu(migratePublish 트랙보존+ptt재생성+롤백, ⚠브라우저 미검증). A1 0x1103=Android 잔존으로 블록. `40c6465` + 하니스 `0ee52be` |
