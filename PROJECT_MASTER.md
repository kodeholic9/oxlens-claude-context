...
description: |
  ...
  wire v3 (8B 헤더 / 16진 opcode), WireAckState, User Probe,
...
| `guide/REGRESSION_GUIDE_FOR_AI.md` | 회귀시험 | oxe2epy 회귀 운용 (`python -m oxe2epy run-all` 정식 게이트) |
...
| 항목 | 로컬 경로 | 설명 |
|------|-----------|------|
| 서버 | `/Users/tgkang/repository/oxlens-sfu-server` | Cargo workspace (common + oxsig + oxrtc + oxsfud + oxhubd + oxcccd + **oxadmin** 운영 CLI). 회귀 harness 는 레포 루트 `oxe2epy/`(파이썬 — 구 crates/oxe2e 삭제 완료) |
| 웹 클라이언트 | `/Users/tgkang/repository/oxlens-home` | **활성 SDK = `sdk0.2/`** (배포용 TypeScript, v0.2 백지 재작성 20260701 — tsup ESM+CJS+.d.ts). v0.6 `sdk/`(JS)는 이전 정본(관측 미이식분 참조). 구 core/ = `__core_deprecated__/`(화석) |
| SDK 코어 | `/Users/tgkang/repository/oxlens-sdk-core` | Rust 네이티브 SDK (libwebrtc C++ 엔진 + Rust 코어) + Android Kotlin 바인딩(`platform/android`) — 휴면 |
...
## 웹 클라이언트 구조 (oxlens-home, 활성 SDK = sdk0.2)

→ **[PROJECT_WEB.md](PROJECT_WEB.md)** 로 이동 (2026-06-03 분리, 20260701 sdk0.2 v0.2 갱신). 상세는 그 파일 참조.

---

## 신 SDK 아키텍처 (sdk0.2 — 2-path + internal 봉인. 구 "6 평면+3 확장 훅"은 v0.6 어휘)
...
| 0x1702 | USER_PROBE_REPLY | User Probe 진단 응답 (oxadmin user 실시간 진단 — 클라가 getStats/device/element 수집 후 unicast 회신. 구 TRACK_DUMP_REPLY 재활용, 0620) |
...
| 0x2105 | TRACK_STALLED | 미디어 정체 감지 알림 (ssrc 목록 + 실 track_id, 0703 가짜 합성 id 정정) |
| ~~0x2106~~ | ~~LAYER_CHANGED~~ | **폐기 20260705** — 자동 레이어 전환 통보. emit/dispatch 전무 고아(0621 PLI Governor 재설계로 발신처 소멸). 카탈로그 44→43 |
...
| 0x3002 | ADMIN_SNAPSHOT | 어드민 스냅샷 (방 + User 뷰). hub admin REST room snapshot + oxadmin room snapshot |
...
| 0x3005 | ADMIN_DOWNLINK_INJECT | 자동 레이어 신호 주입 훅 (admin→sfud, remb_bps/loss_fraction 기록만 — 판단·집행은 정규 tick. auto_layer=off 시 거부, 20260709) |
...
서버 버전 / 구현 완료 / 할 일 등 진행 상태는 전부 `context/SESSION_INDEX_YYYYMM.md`(당월) + `context/YYYYMM/` 세션 파일을 **단일 출처**로 한다. 본 마스터 문서에는 절대 중복 기록하지 않는다. 의심 시 `context/YYYYMM/` 파일 직접 확인을 SESSION_INDEX 조회보다 우선.
...
큰 단계의 완료/진입만 한 줄 단위로 박는다. 세부 진척은 `context/SESSION_INDEX_YYYYMM.md`(월별) 단일 출처.
...
- ✅ **묶음 8 운영성 마무리** (2026-05-17, `20260520a`) — F26 ingress_mbcp.rs dead 파일 청소 (-107줄) + 주요 모듈 5개 `//!` 표준화 + PROJECT_MASTER E-1 명세 7항목 반영. 194 PASS.
...
- ✅ **oxe2e 백지 재설계 (파이썬 oxe2epy)** (2026-06-27) — 전체 파이썬 2층(aiortc ORTC 봇 + 출처분리 검증기). 공리 1~4(출처분리 / 등식 갈래B failability / 단일시계 인과 / 결함주입). 등식 12+ / 신원 5점 / simulcast 회귀가드 5종 / known-defect 격리(quarantine·XPASS 자동해제) / PTT MBCP floor·gating / 인과 타임라인(pandas, priming 19ms→guard 250ms 검증) / 결함주입·시드 결정성 / 전이(⑦화자전환·⑧full→half). 구 Rust `crates/oxe2e` 물리 삭제 완료(`202f692`). 루트 `oxlens-sfu-server/oxe2epy/`(커밋·push 완료, 이후 27등식 26시나리오+RTX gate 레퍼토리로 확장 0705). 상세 §시험 체계.
- ✅ **웹 SDK v0.2 백지 재작성 (sdk0.2, 배포용 TS)** (2026-07-01) — v0.6 `sdk/` 전수분석 → 배포용 TypeScript SDK 백지 재작성. 2-path(`engine.join` 단일방 / `engine.talkgroups` 다방·`talk`) + 어휘 계승(Engine/Room/Endpoint/Pipe/floor/power) + internal 봉인(wire/transport/sdp/mbcp) + **IoC 훅 릴레이 제거**(RoomHost 직접 호출) + 느슨 TS·tsup(ESM+CJS+.d.ts). 물리 계약(publish 3단계/TRACKS_READY/codec/복구 R1·R2/표시) 보존. **cross-sfu PTT slot GAP 해소**(`virtual._byMid` 폐기→`slotByMid`+engine sfuId 라우팅). 3층 라이브 회귀 5/5 통과 + main 병합. 관측 계층(telemetry/user-probe)은 v0.3. 상세 [PROJECT_WEB.md](PROJECT_WEB.md) + `sdk0.2/DESIGN.md`.
- ✅ **Hyb 1PC/2PC 선택형 (ConnMode + pcMode)** (2026-07-09) — Phase 0 교대협상 스파이크(PASS 150/0) → 서버(ConnMode·1PC RTCP 라우터·egress_session·MidPool base 32) → 클라(sdk0.2 pcMode·단일 협상 큐·buildUnifiedRemoteOffer 분리형 ssrc·자동 2pc 폴백) → 통합검증(1PC 라이브 5종 + 2PC 무손상 3층 17/17·2층 26종·cargo 226). 2PC 경로 diff 0(모드 게이트 뒤). 상세 [PROJECT_SERVER.md](PROJECT_SERVER.md)·[PROJECT_WEB.md](PROJECT_WEB.md) + `202607/20260708_hyb_1pc_design_done.md`.
- ✅ **Simulcast 자동 레이어 전환 v1+v2** (2026-07-09~11) — v1 REMB DownlinkController(비대칭 히스테리시스·grace·backoff) + v2 TWCC send-side BWE(egress 스탬핑·GccEstimator·RTX 패딩 프로브) + 운영 3단 사다리 policy.toml `auto_layer=off|v1|v2`(fallback off, opt-in). 라이브 A/B·dummynet 물증·프로브 3회 수렴 promote 실측. 전 커밋 push. 상세 [PROJECT_SERVER.md](PROJECT_SERVER.md) + `202607/20260711_auto_layer_final_report.md`.
...
- `Counter`: AtomicU64(flush=swap 0) / `Gauge`: AtomicU64(flush=peek) / `TimingStat`: 별도 AtomicU64 4개(sum_us/count/min_us/max_us, CAS min/max — 단위 us)
- `Registry<T>`: OnceLock wrapper, static 접근
- `metrics_group!` 매크로: 카테고리별 구조체 자동 생성 + `flush()` JSON nested 구조 (필드 doc 주석 불허 — 일반 주석만)

### SfuMetrics (oxsfud, static Registry)
- 카테고리 11: srtp, pli, rtcp, bwe, nack(flat merge), relay, track, codec, ptt, dc, **simulcast**(auto layer demote/promote/backoff)
...
### 클라이언트 수집 (TELEMETRY 0x1302 — **hub 종착 → oxcccd 직행**, sfud 미경유. 진단 경로 일원화 0620. 구 sdk/observability/telemetry.js 는 v0.6 — sdk0.2 관측 계층은 v0.3 백로그)
...
- **SDK 타입/빌드 검증 (sdk0.2)**: `tsc --noEmit`(매 계층) + `tsup` 빌드(ESM+CJS+.d.ts). 커밋 전 통과 의무. (구 sdk/ node mock `sdk/tests/*_check.mjs` 20종은 v0.6 기준.)
...
## SDK 코어 (oxlens-sdk-core — 휴면)

**Rust 멀티크레이트 네이티브 SDK** (libwebrtc C++ 미디어 엔진 + Rust PTT 로직): `crates/oxlens-core`(client/sdp/signaling) + `oxlens-webrtc` + `oxlens-webrtc-sys` + `oxlens-bench`. `oxlens-jni` 는 폐기(DEPRECATED).
- Android 는 `platform/android/` Kotlin 바인딩 — Jetpack Compose UI + Camera2 + Twilio AudioSwitch + PTT FloorFsm
- Telemetry (S/A/B/C + events + PTT diagnostics), 2PC SDP, Publish+Subscribe, Mute 3-state
- libwebrtc custom AAR (AudioInterceptor 패치 — 구체 브랜치·패치 규모는 재개 시 재확인)
...
- **flat scope API** (`engine.affiliate()` / `engine.select()` 등) — 네임스페이스 그룹이 정답(`select` 단어 DOM/SQL 자동완성 혼동, Engine 비대). 구 `engine.scope.*` → 현 **`engine.talkgroups.*`**(affiliate/join/select/leave, sdk0.2) 확정
...
- `.cargo/config.toml`: `rustflags = ["--cfg", "tokio_unstable"]` (Tokio RuntimeMetrics API 활성화 — LiveKit 등 프로덕션 SFU 패턴)
- Config: system.toml + policy.toml (.env / dotenvy 삭제됨)
- protobuf: `brew install protobuf` (tonic-build codegen)
- 풀 클린 빌드: 10~15분 / 증분: 수초
- 웹 SDK(sdk0.2): TypeScript(느슨, strict:false / strictNullChecks:true), tsup(ESM+CJS+.d.ts), 런타임 무의존. 앱/UI: Vanilla JS ES modules, Tailwind CSS, Phosphor Icons
...
- **"코딩해" 명시 시에만 코드 작성** — 그 전엔 설계/질문만. 질문·의문·지적은 코딩 지시가 아니다(답만, 코드 변경 금지).
- 코딩 전 **영향 범위 먼저 설명**
