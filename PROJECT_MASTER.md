...
| 항목 | 로컬 경로 | 설명 |
|------|-----------|------|
| 서버 | `/Users/tgkang/repository/oxlens-sfu-server` | Cargo workspace (common + oxsig + oxrtc + oxsfud + oxhubd) |
| 웹 클라이언트 | `/Users/tgkang/repository/oxlens-home` | Vanilla JS + Tailwind (Core SDK v2 + demo) |
...
└── crates/
    ...
    │       │   ├── room_id.rs      — RoomId 타입
    │       │   ├── scope.rs        — RoomSetId + RoomSet (Cross-Room rev.2 scope 모델, RCU + ArcSwap)
    │       │   ├── participant.rs  — Participant (DuplexMode, TrackType, Track, role:u8, MidPool, ParticipantPhase)
    │       │   ├── peer.rs         — Peer / PublishContext (pub_room/pub_stats + extmap atomic 4종 + audio_mid ArcSwap + audio_duplex/twcc home) / SubscribeContext / DcState + sub_rooms + PeerMap SubscriberIndex. publish_room() 단일 진입 헬퍼
    │       │   ├── floor.rs        — FloorController (PTT 발화권, QueueUpdated 액션)
    │       │   ├── floor_broadcast.rs — Floor 액션 → DC/WS 브로드캐스트 (SubscriberIndex 기반 user 단위, speaker_rooms/via_room TLV)
    │       │   ├── ptt_rewriter.rs — SSRC/seq/ts 오프셋 리라이팅 + translate_rtp_ts + is_pending_keyframe
    │       │   ├── stream_map.rs   — StreamKind enum 만 잔존 (4-way 분류 Audio/Video/Rtx/Unknown). 구 MediaIntent/VideoSource struct 폐기 (catch 2 3-F, 0529) — 트랙 메타는 PublisherTrack(물리) / extmap 은 PublishContext atomic
    │       │   ├── speaker_tracker.rs — Active speaker 추적 (RFC 6464)
    │       │   ├── subscriber_gate.rs — SubscriberGate (mediasoup pause/resume)
    │       │   └── pli_governor.rs — PLI Governor (인과관계 기반 PLI 평탄화)
    │       ├── media/
    │       │   ├── mod.rs / router.rs — 미디어 라우팅
    │       │   └── track.rs        — 트랙 관리
    │       ├── transport/
    ...
    │       │       ├── egress.rs       — UDP 송신
    │       │       ├── nack_generator.rs — Generic NACK 생성기 (publisher 측 손실 감지, bwe_timer 합류)
    │       │       └── rtcp.rs / rtcp_terminator.rs / twcc.rs / pli.rs / rtp_extension.rs
    │       ├── datachannel/
...
| 0x1003 | ROOM_JOIN | 방 입장 → server_config 수신 + sub/pub auto-select (옵션 A) |
| 0x1004 | ROOM_LEAVE | 방 퇴장 |
| 0x1005 | ROOM_SYNC | 참여자+트랙+floor 전체 동기화 (폴링) |
| 0x1101 | PUBLISH_TRACKS | 트랙 의도 (duplex/simulcast 명시, 증분 add/remove). PublishContext atomic store + PublisherStream/placeholder 등록 (구 MediaIntent struct 폐기, catch 2 3-F) |
...
| 0x1200 | SCOPE | sub_rooms 변경 (body `mode: "update"` 증분 / `"set"` 전체 분기). pub_room 은 join 시 auto-select, SCOPE 가 건드리지 않음 |
...
| 0x2101 | TRACKS_UPDATE | 트랙 추가/제거/duplex_changed (per-user mid 포함) |
...
| 0x2200 | SCOPE_EVENT | scope 변경 완료 통보 (`sub_set_id` / `sub` / `pub_rooms` (단수 — 1-element vec or 빈 vec) / `cause` / `change_id`). SCOPE 응답 + broadcast 공용 |
...
| 0x2500 | ACTIVE_SPEAKERS | Active Speaker 목록 (RFC 6464, ACK 없음 — 빈도 높음) |
...
| 0x3002 | ADMIN_SNAPSHOT | 어드민 스냅샷 (방 + User 뷰) |
...
- PUBLISH_TRACKS 처리 = PublishContext atomic store (extmap 5 + audio_mid/audio_duplex) + PublisherStream 등록
- Non-sim: PUBLISH_TRACKS 시점 실 SSRC PublisherStream 등록 (track_ops 전담)
- Simulcast: **PUBLISH_TRACKS 시점 placeholder PublisherStream 사전등록** (sentinel ssrc `0xF000_0000|counter`, simulcast=true, rid=None, mid/source/pt 박힘). RTP 도착 시 `rid` 파싱 → 같은 source 의 placeholder remove + 실 ssrc PublisherStream rid 별 분화 등록 (`[STREAM:PROMOTE]` 로그 1회). mediasoup `NewRtpStream` rid 별 발급 / LiveKit TrackInfo 사전등록 정합
- resolve_stream_kind 판별: Opus PT=111 → rid → repaired-rid → PublisherStream.actual_rtx_pt → MID. **extmap ID 는 PublishContext atomic load**(rid/repair_rid/mid/audio_level/twcc). **MID↔kind 매핑 = PublisherStream.mid + PublishContext.audio_mid** (race 안전망). **핫패스 `stream_map.lock()` = 0건** — 매 RTP lock+clone allocation 폐기
- RTX 는 PublisherStream.actual_rtx_pt 매칭 또는 `repaired-rtp-stream-id` extension
...
- subscriber SDP re-nego 완료(TRACKS_ACK) → gate resume + GATE:PLI
...
- RTX(PT=97)는 recv_stats/send_stats에서 반드시 제외

### SR Translation
- Conference/Simulcast: SSRC/RTP ts 원본 유지, packet_count/octet_count만 egress SendStats 기준
...
### STALLED 감지
- `packet_count_at_ack` 스냅샷: TRACKS_ACK 시점 send_stats.packets_sent 기록
...
서버 버전 / 구현 완료 / 할 일 등 진행 상태는 전부 `context/SESSION_INDEX.md` + `context/202604/` 세션 파일을 **단일 출처**로 한다. 본 마스터 문서에는 절대 중복 기록하지 않는다. 의심 시 `context/YYYYMM/` 파일 직접 확인을 SESSION_INDEX 조회보다 우선.
...
- **publish 자료 home = `PublishContext`** — `pub_room` (선택된 발언 방, 묶음 3 단수화) + `pub_stats` (publisher 측 카운터/타이밍) + **PC pair 협상 메타** (`rid/repair_rid/mid/audio_level/twcc` extmap ID `AtomicU8` + `audio_mid: ArcSwap<Option<Arc<str>>>` + `audio_duplex: AtomicU8`, catch 2 3-B/3-C 0529) 모두 `PublishContext` 소유. SDP 협상 결과는 PC pair 단위 — 트랙 메타는 `PublisherTrack`(물리, SSRC) 흡수 별도. PLI 자료(`pli_state`/`simulcast_pli_pending` 등)는 논리 `PublisherStream`(source 단위) home — 2계층 전환(2026-05-30, 명제 B 해결). `Peer::publish_room()` 단일 진입 헬퍼만 노출. publisher 측 = `peer.publish_room()` / subscriber 측 = `peer.sub_rooms` 임의 1방 의미 분리 (`first_room_hint` 헬퍼 폐기, 0521c)
...
```rust
Peer {
    joined_rooms: HashSet<RoomId>,                  // 입장한 방들 (peer.rooms)
    sub_rooms:    ArcSwap<RoomSet>,                  // "받을 방들" = MCPTT Affiliated (N방 청취)
    publish: PublishContext {
        pub_room: ArcSwap<Option<RoomId>>,           // "보낼 방" = MCPTT Selected (1방 발언, 묶음 3 단수화)
        // pub_stats, srtp, sdp, recv_stats 등 PC pair publisher 자원 home
    },
    // ...
}
struct RoomSet { set_id: RoomSetId, rooms: HashSet<RoomId> }  // sub_rooms 전용 (묶음 1 의미 축소)
```

- **외부 접근은 `peer.publish_room() -> Option<RoomId>` 단일 진입 헬퍼만** — `publish.pub_room` 직접 접근 금지 (Peer 재설계 원칙 — publish 자료 home 일원화)
- **set_id** 세션 수명 불변, 형식 `"sub-{user_id}"`. pub 측 set_id 폐기 (1방 발언이라 식별자 불필요)
...
// 상태 조회 — pub 은 단수 의미 (0/1-element Set)
engine.scope.{sub, pub, sub_set_id, snapshot(), hasSub, hasPub}
engine.scope.on('changed', ({ sub_set_id, sub, pub, cause, change_id }) => ...)
```
...
**JSON key**: SCOPE wire body `"sub"` array 만 의미. SCOPE_EVENT broadcast `pub_rooms` 는 0/1-element Vec (pub_room 단수의 wire 표현, serde rename JSON `"pub"`). `pub_set_id` 는 빈 string 직렬화 (식별자 불필요).
...
- **set_id 는 세션 수명 불변** — scope 내용이 바뀌어도 유지. reconnect shadow 복원 호환
...
- **비적용 범위**: Rust/JS 단위 테스트(`cargo test`, `*.test.mjs`), 성능 부하(oxlens-sfu-labs), Android SDK — 별도 체계.
...
- **`_publishCamera` resume은 항상 publishTracks 재전송**
- **PC는 Engine 소유 (Room 아님)** — cross-room 시 sub PC 공유
