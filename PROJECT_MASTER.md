...
카테고리 nibble 하나로 dispatch / 방향 / pid 부여 / ACK / priority / intent 전부 lookup. flag 비트 신설 금지가 v3 도그마. 총 **44 op** (ADMIN_REAP `0x3004` 추가 20260613 / TRACK_DUMP `0x2701`·`0x1702` → USER_PROBE 재활용 20260620 — 번호 유지·의미 교체). 새 op 추가 시 `oxsig::opcode::ALL_OPS` 갱신.
...
서버 버전 / 구현 완료 / 할 일 등 진행 상태는 전부 `context/SESSION_INDEX_YYYYMM.md`(당월 — 현 `_202606`) + `context/202606/` 세션 파일을 **단일 출처**로 한다. 본 마스터 문서에는 절대 중복 기록하지 않는다. 의심 시 `context/YYYYMM/` 파일 직접 확인을 SESSION_INDEX 조회보다 우선.
...
- ✅ **oxe2e 백지 재설계 (파이썬 oxe2epy)** (2026-06-27) — 전체 파이썬 2층(aiortc ORTC 봇 + 출처분리 검증기). 공리 1~4(출처분리 / 등식 갈래B failability / 단일시계 인과 / 결함주입). 등식 12+ / 신원 5점 / simulcast 회귀가드 5종 / known-defect 격리(quarantine·XPASS 자동해제) / PTT MBCP floor·gating / 인과 타임라인(pandas, priming 19ms→guard 250ms 검증) / 결함주입·시드 결정성 / 전이(⑦화자전환·⑧full→half). 구 Rust `crates/oxe2e` 폐기 전제. 루트 `oxlens-sfu-server/oxe2epy/`. 미커밋(누적 3.5~4 결재 대기). 상세 §시험 체계.
...
- **구 oxe2e (Rust, `crates/oxe2e`, 2026-05-30~)**: **oxe2epy(파이썬)로 재설계 대체 — 폐기 전제.** 개념 자산(등식·음성 픽스처·식별 축)만 oxe2epy 로 이식. 물리 삭제 커밋은 미정. 가이드 `context/guide/REGRESSION_GUIDE_FOR_AI.md` 는 Rust oxe2e 기준(파이썬 oxe2epy 가이드는 미작성 — 별 토픽).
...
- **3층 라이브 회귀 (oxlens-home/qa/live, Playwright, 20260630 신설 · 대상 = sdk0.2)**: 실브라우저 × 트랙단위 3권위 교차(`qa.tracks` / `qa.trackStats` / `oxadmin room`). 시나리오 5종(CONF-VIDEO/SIMULCAST/REPUBLISH/DUPLEX/MULTIROOM, 20260701 5/5 통과). `window.qa`(page.evaluate) 제어 — 단일 page=단일 user. `qa/qa.js` = v0.2 어댑터(window.qa 표면 v0.1 계약 보존 → spec 무변경). http-server(5599) 자체기동(★Live Server 금지). 실행 `cd qa/live && npm test`. 상세 = `qa/README.md`. (구 `__qa__` controller/iframe UI 폐기.)
- **핵심 원칙 3 (qa/README.md)**:
...
```
context/
├── 202603/ ~ 202606/  ← 월별 세션 파일 (YYYYMM — 작업지침 + 완료보고 + 설계 일부 혼재)
...
└── SESSION_INDEX_202606.md  ← 세션 인덱스 (월별, 루트 유지)
```
