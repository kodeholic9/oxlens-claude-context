...
### 0-4. 시각 정합
- ~~클라(e2e 러너) 타임라인이 UTC~~ → **해소(0613)**: 러너 `fmtT` 를 로컬 시각으로 통일(서버 LocalTimer 와
  그대로 대조 가능). 원인은 `toISOString().slice(11,23)` — UTC 인데 Z 표식까지 잘려 무표기 출력.
- 교훈은 유효: **타임스탬프를 대조하기 전에 양쪽 타임존 출처부터 확정**(브라우저 devtools 콘솔=로컬,
  `toISOString`=UTC, 서버=KST). 의심스러우면 시각 대신 **클라가 만든 방 이름**(`e2e_conf_xxxxx`)으로
  서버 로그 grep — 타임존 무관해서 더 정확.
