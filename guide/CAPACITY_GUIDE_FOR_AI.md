...
**성능시험 계열에서 우리 위치** — 올리는 축으로 갈린다:

| | 올리는 축 | 우리 도구 |
|---|---|---|
| `load` / `stress` / `spike` | 동시 부하량 · 한계 초과 · 급변 | **capacity(이 문서)** — N 스윕 |
| `soak` | **시간·반복** | `soak_runall.sh` → **`REGRESSION_GUIDE_FOR_AI.md` §1-S** |

`soak` 는 약자가 아니라 "오래 담가둔다"는 영어 단어다. 부하를 올리는 게 아니라 `run-all` 을 N회
반복해 저빈도·누적 결함(누수·상태 오염)을 끌어내는 것이라 **회귀 계열**이고, 절차는 회귀 가이드에 있다.
capacity 와 헷갈리지 말 것 — capacity 는 pass/fail 이 없고(곡선), soak 는 **결함 0** 이 판정이다.
