...
4. **★층 경계 — 결정적으로 만들 수 있으면 2층이 본다. 3층은 그 성질을 단언하지 않는다.**(20260819 신설)

#### 규율 4 — 결정적이면 2층. 3층의 중복 단언이 게이트를 흔든다

**층은 능력으로 갈린다.** 2층은 신호를 **위조해 통제**할 수 있어 결정적이고, 3층은 실브라우저·
실인코더·실추정기 위에서 도니 **타이밍이 본질적으로 비결정적**이다. 같은 성질을 둘 다 단언하면
약한 쪽(3층)이 흔들려 게이트 전체의 신뢰를 깎는다.

- **2층이 볼 수 있으면 2층이 본다.** 신호 계층 주입으로 결정적이 되는 것 — BWE/층 전환·
  RTCP 반응·전이 순서·라우팅·신원.
- **3층은 봇이 원리적으로 못 보는 것만 쥔다** — 실디코더/NetEQ/jb, 즉 사람 눈과 귀에 닿는 것.
- 3층이 2층 영역을 단언해야 할 것 같으면, **그건 2층에 등식이 없다는 뜻**이다. 3층에 술어를
  얹지 말고 2층에 등식을 세워라.

**실증(20260819 — `SIM-AUTO-01`)**: 3층이 "주입 → 10초 안에 auto_cap=l"을 단언했는데,
그 신호원(TWCC 추정기)의 수렴 시점이 비결정적이라 환경이 두 상태로 갈렸다.
- 수렴 후 진입 → 이미 `l` → **끝 상태만 보는 판정이 그 자리에서 통과**(공허한 초록)
- 수렴 전 진입 → `h` → 10초 안에 안 내려감 → **빨강**
15회 누적에서 baseline `l` 12회 전부 통과 / `h` 3회 전부 실패 — 완전 분리(우연 확률 1/455).

같은 성질을 **2층 `sim_bwe_updown` 이 이미 결정적으로 본다.** 봇이 TWCC FB 의 **도착시각
숫자만** 위조하면 서버 추정기가 반드시 overuse 를 본다(기울기 산정: `slope×gain(4)×window(20)
> threshold(12.5)` → 187ms/s 필요 → 200 사용). 실망 조작·sudo 불요, 매번 같은 답.
→ **3층의 그 단언은 중복이었고, 중복된 쪽이 흔들렸다.**
