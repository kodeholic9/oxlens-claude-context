...
상태: 2단계 완전 종료 (Phase A~F · 2층 10/10 · 3층 통과 · 커밋 5e880b5 · origin/main 반영). <!-- [20260722 정정] 로컬 추적 main=origin/main=5e880b5 → push 반영 확인. push 실행 주체=부장님 -->
...
* ★ **커밋 완료** — 5e880b5 (oxsfud/oxhubd/oxsig/Cargo(oxsfud·oxhubd) 전체, 1단계 cdcc9d5/dd146dc 위에 얹힘).
  <!-- [20260722 정정] 구 "커밋 대기/미커밋 누적"은 작성 시점 상태 — 이후 5e880b5 로 커밋·push 반영(로컬 추적 main=origin/main). 커밋·push 실행 주체=부장님. A-1 규명: 상단 상태 줄(커밋됨)이 사실, 본 줄의 "대기"가 stale 이었음 -->

---

## §8 산출물 보완 (20260722 종결 지침 A-5 — 착수 시점 미충족분 소급)

> 본 절은 20260722 시그널링 종결 지침 Phase A-5 에 따라 김과장이 append. 근거는 상단 실행 기록 + git 실측.

### 개명 타입 전수 (구 oxsfud/signaling/message.rs 17종 → oxsig::message)

구 정의 17종 = 요청 15 + 응답 2(RoomEventPayload/ScopeEventPayload). 실측: `git show 5e880b5^:crates/oxsfud/src/signaling/message.rs`.

| 처분 | 수 | 구 → 신 |
|---|---|---|
| 요청 개명 | 11 | RoomCreateRequest→RoomCreateReq · RoomJoinRequest→RoomJoinReq · RoomLeaveRequest→RoomLeaveReq · PublishTracksRequest→PublishTracksReq · MuteUpdateRequest→MuteUpdateReq · MessageRequest→MessageReq · CameraReadyRequest→CameraReadyReq · TracksReadyRequest→TracksReadyReq · SubscribeLayerRequest→SubscribeLayerReq · ScopeUpdateRequest→ScopeUpdateReq · ScopeSetRequest→ScopeSetReq |
| 응답 개명 | 2 | RoomEventPayload→RoomEvent(Phase B) · ScopeEventPayload→ScopeEvent(Phase D) |
| 동명 유지 | 3 | PublishTrackItem · TrackStateReq(oxsig판은 room_id 필드 추가, sfud 미파싱) · SubscribeLayerTarget |
| 청산(고아) | 1 | IdentifyRequest — 참조 0(survey §D1), oxsig 대응(IdentifyReq)은 존재하나 sfud 소비 없음 |

계 17 = 개명 13 + 동명 3 + 청산 1.

### 도메인별 조립부 수 (착수 전 실측 line 165 집계 + Phase 기록)

| 도메인(파일) | Phase | 조립/파싱부 |
|---|---|---|
| session (oxhubd ws/mod.rs·rest/admin.rs) | E | HELLO·IDENTIFY_RESULT·TOKEN_REFRESH(2형)·SESSION_DISCONNECT·RECONNECT |
| room (room_ops·helpers·tasks) | A/B | 요청 파싱 JOIN/CREATE/LEAVE/SYNC + 응답 4(LIST/CREATE/LEAVE + SYNC) + ROOM_EVENT 4발신처(join·leave·helpers evict·tasks reaper) |
| media (track_ops·helpers·ingress_publish·tasks·domain/room) | C | 요청 파싱 PUBLISH/READY/MUTE/CAMERA/SUB_LAYER/TRACK_STATE_REQ + 응답 6 + 이벤트 4(TRACK_STATE/VIDEO_SUSPENDED/RESUMED/STALLED) + TRACKS_UPDATE **5발신처** + 헬퍼 완전 타입화(codec/extmap policy·member_infos·build_*·collect_subscribe) |
| scope (scope_ops·helpers) | D | SCOPE 응답 + scope_snapshot 헬퍼 |
| data (room_ops·client_event) | D | MESSAGE 응답/MESSAGE_EVENT + CLIENT_EVENT 파싱(3필드 카운팅, ts/detail 파싱만) |
| ext (oxhubd moderate/handler·session) | E | MODERATE_EVENT 5형(authorized/unauthorized/speakers_updated/queue_update/phase_changed) + USER_PROBE_REQ + speakers_list/queue_snapshot |
| admin (oxsfud admin·oxhubd rest) | E | ADMIN_SNAPSHOT(전체 2형)·ADMIN_REAP·ADMIN_DOWNLINK_INJECT |

집계(line 165 실측): 요청 소비처 **9 핸들러 파일** · 응답/이벤트 조립 **Packet::ok/new 31곳**(oxsfud) + tasks/ingress 인라인 + hub 인라인(ws 12·moderate 6·admin). oxrtc body 미사용(범위 밖).

### SESSION_INDEX 에만 있고 본 실행 기록에 없던 문서 발견 2건 (기재만 — 처분 부장님)

SESSION_INDEX 07-21 요약 꼬리에 "문서 발견 2건 … 부장님 정정 판단"으로 적혔으나 본 파일 실행 기록엔 없었음. 종결 지침 A-5 로 소급 기재:

1. **SCOPE_EVENT pub 단수 미반영** — SCOPE_EVENT(0x2200) 관련 문서/타입에서 `pub`(발언 채널) 표기가 단수로 남아 갱신 안 됨. 발신처 0(미emit, S-d 설계 대기, survey)과 함께 20260722 종결 지침 E-2 SCOPE_EVENT 항에서 현행 상태 재확인 대상.
2. **3층 spec 수 14→15** — 3층 라이브 게이트 spec 수가 문서상 14 였으나 실제 15(본 파일 line 340 "전체 15 spec"). 마스터/설계 문서의 3층 spec 수 표기 갱신 필요.
