...
> 본문 무수정. §9 표의 **M0 을 닫는다**(부장님 결재 2026-08-19).

**결정: 3축 중 (ii) 전노드 복제 채택.** 기전이 구현됐다 — SFU 가 방 lifecycle 을 발행하고
hub 전부가 사본을 유지한다(권위=SFU, hub 는 파생). 근거는 `202608/20260818a_room_lifetime_task.md`
§7-7, 커밋 `f6599d6`·`d6c9bb8`.

| | 기전 |
|---|---|
| 생성 | `ROOM_EVENT{room_created}` → hub `learn_room(room_id, sfu_id)` |
| 소멸 | `ROOM_EVENT{room_destroyed}` → hub `unbind_room` |
| 늦게/다시 뜸 | 구독 직후 `reconcile_sfu` — 그 sfu 에 ROOM_LIST 를 물어 채운다 |

"채팅방 시나리오와 함께 결정"이라던 조건은 해제한다 — 채팅방 fanout 을 안 건드리고도
디렉토리만 독립적으로 설 수 있음이 구현으로 확인됐다.

### ★남는 전제 — 방 중복 금지는 디렉토리가 보장하지 않는다

§8.2 의 CAP 경고는 유효하다. 지금 충돌 처리는 **탐지**(먼저 안 쪽 유지 + `[ROOM:CONFLICT]`
error)이지 **예방**이 아니다. 두 hub 가 같은 방을 동시에 만들면 각자 자기 sfu 에 만들고,
서로의 통보를 받은 뒤에야 어긋남을 안다 — 그때는 이미 미디어가 갈려 있다.

**지금 중복이 안 나는 이유는 디렉토리가 아니라 HRW 가 순수 함수라 모든 hub 가 같은 답을
내기 때문이다.** 그 성질은 `sfu_ids` 가 고정일 때만 유효하다.

→ **재개 조건**: 발견 계층(Zenoh)이 들어와 `sfu_ids` 가 시간가변이 되는 시점, 또는 방
중복이 실제로 관측되는 시점. 그때 (i) 단일 결정자나 (iii) 경량 합의를 다시 꺼낸다.
그 전까지 M0 은 닫힌 것으로 본다.
