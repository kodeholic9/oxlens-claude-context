...
### 3. ★★협상 축이 **세 층 전부**에서 비어 있다 (20260817 확정 — 다음 세션 최우선)

> 부장님: *"검증 항목이 모자라진 않다는 기계적인 답변만 들었다."*
> 답이 기계적으로 나온 이유가 여기 있다. **항목 수는 충분한데 축 하나가 통째로 없다.**

오늘 근인(PT 협상)뿐 아니라 STALLED(0711)·SCTP 누수(0814)도 전부 **봇이 안 밟는 경로**에서 났다.
항목으로 세면 46개인데 **경로로 세면 협상 갈래를 아무도 안 탄다.**

| 층 | 협상 축 상태 | 근거 |
|---|---|---|
| **1층 단위** | 시험 **7개가 존재하는데 실행되지 않는다** | `sdk0.2/test/` 에 `onepc-sdp`·`codecs-sub`·`renego-ready` 등 7개. **`package.json` 에 `test` 스크립트 없음**, devDeps 는 `tsup`/`typescript` 뿐이라 **러너도 없다.** 파일 머리말은 *"Phase 0 spike, PASS 150/0"* — 한때 돌렸고 그 뒤 배선이 끊겼다. 게다가 픽스처가 **VP8(pt 96)뿐**이라 돌았어도 오늘 결함(H264 profile)은 못 잡는다 |
| **2층 봇** | **SDP 자체가 없다** | `bot/transport.py` 머리말 *"aiortc ORTC 저수준 조립 (SDP-free 정합)"*. `bot.py:56` 이 그걸 **장점으로** 적어뒀다 — *"봇은 ORTC 직결(SDP 없음)이라 클라 SDK 의 mirror-offer 난제가 없다"*. ★**그 "난제 없음"이 곧 구멍이다** — 클라가 겪는 난제를 시험이 안 겪으니 그 난제의 결함을 못 잡는다. 오늘 사고가 정확히 mirror-offer/answer 합성에서 났다 |
| **3층 브라우저** | 협상은 타는데 **실패해도 규명 자료가 안 남았다** | 20260817 진단 배선으로 해소(§C) |

**각 층은 자기 축에서 충분했다. 그래서 "모자라지 않다"가 나왔다.** 세는 단위가 틀렸다.

#### 착수 (비용순 — 앞의 둘이 오늘 결함을 직접 덮는다)

1. **1층 배선 복구** — `sdk0.2` 에 러너(vitest 또는 tsx+node:test) devDep 1개 + `"test"` 스크립트 1줄.
   시험 7개가 `.ts` 라 로더가 필요하다. **가장 싸고 즉효.** 복구 후 7개 통과 여부부터 확인
   (한때 PASS 150/0 이었으나 그 뒤 소스가 바뀌었다 — 깨져 있을 수 있고, 깨졌으면 그것도 수확)
2. **H264 profile 픽스처 추가** — 오늘 결함의 음성 픽스처. `_mapCodecsToOfferPts` 에
   `42001f`/`42e01f`/**offer 에 H264 여러 개** 케이스. **순수 함수라 브라우저·서버 불요.**
   오늘 수리가 이걸로 고정되고, 되돌리면 빨개져야 한다(갈래B)
3. **경계 명문화** — `qa/README.md:26` 의 *"와이어로 결판나면 2층에서 끝"* 을 확장:
   **SDP 문자열로 결판나면 1층(단위)에서 끝.** 2층은 SDP-free 를 유지하는 게 맞다
   (canned RTP 가 simulcast/PTT 형상 자유의 토대 — `rtp_tx.py:2`). 봇에 SDP 를 붙이면 그 자유를 잃는다.
   **협상은 1층, 와이어는 2층, 실 디코딩은 3층**으로 축을 배분한다
4. 그 다음에야 §0-I 커버리지를 **경로 기준**으로 다시 센다. 지금 수치(등식 46/시나리오 47)는
   **항목 기준**이라 협상 갈래 부재를 못 드러낸다

#### 부수 — 2층 `codec_match` 는 이미 있다

수신 RTP pt ↔ 약속 `video_pt` 대조. 못 잡은 이유는 봇이 `publish_video(pt=96, codec="VP8")` 로
**선언하고 그대로 보내** 항상 일치하기 때문. 등식 신설은 불요이고, **선언과 실송신을 어긋나게 만드는
형상**(악조건 레퍼토리 하나)을 주면 기존 등식이 잡는다. 20260816 §0-I 규율 3의 교과서적 사례.
