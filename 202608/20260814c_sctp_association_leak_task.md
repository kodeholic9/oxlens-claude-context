...
### 검증 · 20260815 08:30~08:41 — 2층 통과, 누수 0

부장님 재기동(08:29) 후 `python -m oxe2epy run-all` 1회. 로그가 일별 단일 파일이라
**재기동 지점 라인 마크**(sfu-1 65147 / sfu-2 59552)를 찍고 그 이후 구간만 셌다.
캡처 일체: `testlogs/202608/20260815_leakfix_before/`
(`before.txt` · `sfu1/sfu2.sample.txt` · `after_restart_mark.txt` · `runall_after.txt` · `after.txt`).

| 지표 | 수리 전 (soak 15회 누적) | 수리 후 (run-all 1회, +90초 시점) |
|---|---|---|
| 회귀 | 43종 OK 43 | **43종 OK 43 / 이상 0** (exit 0) |
| 좀비 association | **9개 생존** | **0** (sfu-1·sfu-2 모두) |
| SCTP 루프 started / ended | started > ended (잔존) | **57/57 · 48/48 — 전부 회수** |
| 취소 경로 발동 | 없음(코드 부재) | **8건** (sfu-1 6 · sfu-2 2) |
| CPU (12분 경과) | sfu-2 127:44, 코어 1개 100% 스핀 | **9.4s / 6.1s**, 실행 중 스레드 **0** |

**판정 근거는 `started == ended`다.** 종전엔 그 차이가 곧 누수였다. 그리고
`[DC] SCTP loop cancelled (peer 회수)` 8건 = **원래 매달렸을 루프 8개를 새 경로가 실제로
걷어낸 것**이다. 종전 생성률(0.18~0.27/run)대로면 이번 1회에서도 1개 안팎이 남았어야 한다.

**+90초 재측정을 따로 한 이유**: 스위트가 *"마지막 `adv_floor_failover` 은 좀비를 남긴 채
끝난다 — 다음 run 전 30초"* 를 스스로 경고한다(sfud 회수 최악 25초). 종료 직후 0 은 회수 대기
구간일 수 있어 신뢰하지 않았다. 회수 창을 넘긴 08:41:18 에도 0 이다.

**부수 관측(회귀 아님)**: `adv_floor_failover` 의 KPI 2 가 botA 1474ms / botB 12823ms 로
1000ms 초과 표시. 이 시나리오는 **급사→ping_timeout 회수→다음 화자**가 설계라 대기가 지표에
들어간 것이고 KPI 1(access)은 4ms 다. 20260814b §미결 4(KPI 2 게이트화)에서 "이 시나리오는
KPI 2 대상 밖"으로 정리할 항목.

### 잔여 (이 수리 이후)

1. **3층 실영상 게이트** — transport 종료 경로 변경이라 미디어 경로는 안 건드렸으나,
   2층 PASS 는 최종이 아니다(부장님 원칙).
2. **soak 재개** — 20260814b §미결 1-② 빈도 측정이 run 16/30 에서 중단됐다(재기동).
   새 바이너리 기준으로 다시 세야 하고, 이제 `좀비=0` 이 기대값이라 판독표도 그만큼 단순해진다.
3. **sfu-1 비대칭 미확정** — 축출 2건 중 1건만 스핀이었던 이유(위 §왜 sfu-1 은 멀쩡한가).
   수리로 증상은 사라졌지만 원인은 안 밝혔다. keepalive 종료 지점 로그 승격이 확인법.
4. **`last_stun` 유휴 판정** — 2차 안전망. peer 가 영영 회수되지 않는 경우(WS 는 살아 있는데
   미디어만 소멸)가 실제로 있는지 재측정 후 결정.
...
## 완료 · 20260816 — 누수 수리 확정, status open → done

`4b8aeb2`(peer 회수 시 DTLS/SCTP 태스크 종료)의 검증이 **두 형상 20회씩**으로 끝났다.

| soak | 형상 | 좀비 |
|---|---|---|
| `soak_20260816_103105` | 직렬 20회 (4시간) | **+0** |
| `soak_20260816_151026` | 병렬 jobs=4 20회 (1시간) | **+0** |

종전 생성률 0.18/run 이면 20회에 3~4개가 붙었어야 한다. 두 번 다 0이고 `seq_loss` 도 0줄이다.
직전 8/15 직렬 20회도 +0 이었다 — **세 번의 20회 표본에서 신규 좀비 0.**

### §잔여 처리

1. ~~3층 실영상 게이트~~ → 20260815 통과(위 §3층 게이트).
2. ~~soak 재개~~ → 위 두 건으로 완료. 20260814b §미결 1-② 의 seq 결손은 **60회 누적 0건**.
3. **sfu-1 비대칭 미확정** — 축출 2건 중 1건만 스핀이던 이유. **미규명 유지**(증상은 수리로 사라졌고
   원인은 안 밝혔다). keepalive 종료 지점 로그 승격이 확인법.
4. **`last_stun` 유휴 판정** — 2차 안전망. 60회 표본에서 좀비 0이라 **필요성 미입증**. 착수 안 함.
5. **`MULTIROOM-01`** → `202608/20260816a` 잔여 3번으로 이관(HRW 로 깨진 cross-sfu 전제, 처방 확정).

**status: open → done.** 남은 3·4는 이 수리의 잔여가 아니라 별건이고, 5는 이관했다.
