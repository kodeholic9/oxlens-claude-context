...
| ~~M0~~ | **종결**(20260819 결재) — (ii) 전노드 복제 채택. 설계서 `20260730_zenoh_discovery_design.md` 부록 참조. ★남는 전제: 중복 금지는 디렉토리가 아니라 **HRW 결정성**이 보장한다(= `sfu_ids` 고정이 전제). 발견 계층 도입 또는 중복 실관측 시 재개 |
