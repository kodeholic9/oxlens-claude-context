...
opened: 20260802
closed: 20260802
refs: [PROJECT_MASTER.md, doclint.sh]
---
# context 기록 규칙 개편 — 평면 배치 · kind 4종 · append 단일 파일 · doclint
...
### 3. `doclint.sh` 신설 — 규칙을 기계로 강제

`context/doclint.sh`. 기본은 기준일(20260802) 이후 신규 파일만 검사(R7 존중), `--all` 로 전수 참고 가능.
...
`./doclint.sh` → **위반 0건**. (개편 전 실행 시 `20260802a` 파일에서 R2·R3 2건 적발 → 수정 후 통과. 검사기가 실제로 무는 것을 확인했다.)

---

## GAP — 못 한 것

1. **과거 712개는 그대로다.** R7대로 개명하지 않았다. `_done` 154개, 접미사 난립 15종이 과거 구간에 남아 있다. `./doclint.sh --all` 은 당연히 대량 실패한다 — 이는 정상이며, 기준일 이후만 초록이면 된다.
...
- `_done` 접미가 하던 "`ls` 로 완료 여부가 보인다"를 잃었다. 대신 frontmatter `status` + 인덱스 status 칸 + `doclint` 가 대신한다. 파일 수는 절반, 한 작업의 맥락은 한 파일.
...
**부장님 관할**(context 레포). 착수 시점에 부장님 기존 미커밋 변경 6건 + 미추적 4건이 이미 있었으므로 **분리 스테이징 권고**.
