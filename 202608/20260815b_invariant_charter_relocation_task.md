...
## 5차 · 20260816 — 2층 병렬 실행 (`2883470`)

부장님 지시: *"2층 시험을 그룹으로 묶어서 동시 진행할 수 있는 방법 고민해바. 11분 이상 걸리는 것을 줄일 방법"*
→ 분석 후 *"1번 진행해"*(시각 술어 목록화) → *"soak 중단하고, 병렬 처리 진행해"*.

### 진단 — 시간이 어디로 가나

47종 `run_secs` 합계 **557s(9.3분)**, 실제 11.5분(오버헤드 2.2분), **최장 단일 55s**.
→ 완벽 병렬화의 바닥은 55초. 4병렬이면 이론상 ~3분.

### 근본 차단은 `user_id` 하나였다

서버 `PeerMap.by_user` 는 **user_id 단일 전역 키**(`peer_map.rs:45`). 실측 공유:
`botB` 46종 / `botA` 41종 / `botC` 17종, 방 이름 `roomX` 8종 등, ssrc `0xA0000001` 30종.
두 시나리오가 동시에 돌면 같은 Peer 를 잡는다 → 원리적 불가.
(ssrc 공유는 서버가 방/peer 범위로 조회해 무해할 가능성이 높으나 **미확인**.)

**해소**: orchestrator 가 방·user_id 에 시나리오 태그. take-over 연출은 시나리오 안에서 값이
같으므로 보존. `run.py` 주석의 *"근본 격리가 아니라 순서 회피"* 가 가리키던 그 근본을 한 것.

### 태그가 드러낸 하니스 버그 2건 — 등식이 잡았다

| # | 버그 | 잡은 등식 |
|---|---|---|
| ① | `SUBSCRIBE_LAYER` 의 `target` 이 봇 id 그대로라 서버가 못 찾음 | `manual_layer_follows` |
| ② | 급사 마킹 `record_floor(a["bot"], ..)` 이 태그 없이 기록 → 화자 구간 미종료 | `gating_correct` |

②가 특히 값지다 — `adv_floor_failover` 가 *"화자 없는 구간 92패킷"* 으로 빨개졌는데,
그 92패킷은 **botA 가 정당하게 발화한 1.50~3.48s 구간**이었다. 소스가 아니라 하니스가 틀렸다.
※ soak 에서 1/9 로 빨갛던 것과 같은 건인지는 **미확인**(위반 서명이 다르다 — 그때는
`floor_failover`+`gating_correct`+`send_honest` 3건).

### 결과

| | 직렬 | 병렬(jobs=4) |
|---|---|---|
| 소요 | 11.5분 | **2분 38초** (4.4배) |
| 판정 | 47종 OK 47 | 47종 OK 47 |

시각 지표 대조: 악화 없음(오히려 최악값이 19→3ms, 24→6ms 로 감소). 여유 50배 이상 유지.

### ★조용한 무효화를 하나 실제로 잡았다 (H1)

`floor_convergence` 가 REQUEST 를 `FLOOR_RACE_S`(100ms) 창으로 묶는데
`conf_floor_contention` 의 간격이 **정확히 100ms = 경계**였다. 지터 1ms 로 101ms 가 되면
클러스터가 깨져 등식이 조용히 skip = 초록이 거짓. **실측으로 재현**(병렬 실행분에서 101ms).
→ `expect_contention` 선언 게이트(클러스터 0 이면 FAIL) + 시나리오 간격 50ms 로 당김.

**교훈**: 병렬화의 대가는 임계 초과가 아니라 **커버리지 소실** 쪽에서 온다.

### 잔여

1. **L5 자원 회수** — 2층 밖 정리 판단 대기.
2. **`adv_floor_failover` soak 1/9 빨강** — 하니스 버그 ②와 동일 건인지 미확인. soak 재측정 필요.
3. **ssrc 공유** 무해성 미확인(방/peer 범위 조회라는 근거는 있으나 실증 안 함).
4. **STALLED 와 `adv_resource`** — flood 가 만든 48건은 오탐이 아니나(진짜 무흐름) 악조건 시나리오라
   정당사유에 넣을지 판단 필요.
5. **push** — 미결재 누적.
