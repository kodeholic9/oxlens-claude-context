...
- **빌드**: `~/repository/webrtc-src` → **`~/repository/src` 로 개명**(0621, DEPS 경로가 `src/` 하드코딩, gclient 솔루션명 `src` 필수) → **`~/repository/webrtc-checkout/src` 로 이동**(0624, gclient 부모 격리). depot_tools(`~/depot_tools`) + `.gclient`(webrtc-checkout/ 부모, managed:False) + `gclient sync`(deps 9.7G) + hooks. CLT-only는 `xcodebuild`/`find_sdk.py` 벽 → **full Xcode 16.2 설치**(macOS 14.6.1 상한, 16.3+는 Sequoia)로 돌파. shim 잔재 `~/repository/xcode-shim/`(폐기).
- **산물**: `~/repository/webrtc-checkout/src/out/Default/video_replay`(arm64). 재빌드 `cd ~/repository/webrtc-checkout/src && PATH=~/depot_tools:$PATH autoninja -C out/Default video_replay`.
